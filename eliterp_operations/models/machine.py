...
STATES = [
    ('operative', 'Operativa'),
    ('operative_failures', 'Operativa prevención'),
    ('repair', 'F/S Temporal'),
    ('out_of_service', 'F/S Total')
]
...
class Machine(models.Model):
    ...
    def _get_horometro_real(self):
        """
        Obtenemos el horometro real (actual) de la máquina
        """
        for record in self:
            cmc_ids = record.cmc_ids.filtered(lambda x: x.state == 'validate')
            if cmc_ids:
                record.horometro_real = cmc_ids[-1]['final_horometro']
                record.last_date = cmc_ids[-1]['date']
            else:
                record.horometro_real = record.horometro_initial
    ...
    state = fields.Selection(STATES, string="Estado", default='operative', track_visibility='onchange')
    image = fields.Binary('Imagen')
    horometro_initial = fields.Float('Horómetro inicial', default=0)
    horometro_real = fields.Float('Horómetro actual', compute='_get_horometro_real', track_visibility='onchange')
    ...
    last_project = fields.Many2one('eliterp.project', 'Proyecto', compute='_get_horometro_real',
                                   track_visibility='onchange')
    ubication = fields.Many2one('eliterp.location', string='Ubicación actual', track_visibility='onchange')
    last_date = fields.Date(compute='_get_horometro_real', string='Fecha de úlitmo CMC', track_visibility='onchange')
